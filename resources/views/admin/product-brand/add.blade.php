         ...
         <div class="col-md-12">
            <div class="card shadow-lg border-0 rounded-3">
               ...
               <div class="card-header bg-light border-bottom-0 py-3 d-flex justify-content-between align-items-center rounded-top">
                  ...
                  @can('slider-list')
                  <a href="{{ route('admin.product-brand.list') }}" class="btn btn-sm btn-sm btn-outline-primary">
               ...
               <div class="card-body p-4">
                  <form id="saveForm" class="theme-form row g-3" enctype="multipart/form-data">
                              ...
                              @if(!empty($product_category_list) && count($product_category_list) > 0)
                                    @foreach($product_category_list as $value)
                                       <option value="{{ $value->id }}"
                                          {{ ($brand->product_category_id ?? '') == $value->id ? 'selected' : '' }}>
                                          {{ $value->category_name }}
                                       </option>
                                    @endforeach
                              @endif
                     ...
                     <div class="col-md-4">
                        ...
                        <input type="file" class="form-control" id="image" {{ empty($brand->image) ? 'required' : '' }} name="image" accept=".jpg,.jpeg,.png"
                           onchange="previewImage(this,'imagePreview')">
                        <div class="mt-2">
                           <div class="mt-3">
                              <a href="{{ !empty($brand->image) ? asset($brand->image) : '' }}" target="_blank">
                              <img id="imagePreview"
                                 src="{{ !empty($brand->image) ? asset($brand->image) : '' }}"
                                 alt="Image"
                                 class="img-thumbnail border rounded shadow-sm p-1"
                                 style="max-height: 100px; {{ empty($brand->image) ? 'display:none;' : '' }}">
                              </a>
                           </div>
                        </div>
                     </div>
                      ...
                      <div class="col-md-4">
                        ...
                        <option value="Active" {{ (!empty($brand) && $brand->status == 'Active') ? 'selected' : '' }}>Active</option>
                        <option value="Inactive" {{ (!empty($brand) && $brand->status == 'Inactive') ? 'selected' : '' }}>Inactive</option>
                        </select>
                     </div>
