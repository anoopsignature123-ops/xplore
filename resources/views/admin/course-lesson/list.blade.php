            ...
            <div class="card shadow-lg border-0 rounded-3">
               ...
               <div class="card-header bg-light border-bottom-0 py-3 d-flex justify-content-between align-items-center rounded-top">
                  ...
                  <div class="d-flex gap-2">
                     ...
                     @can('course-lesson-add')
                     <a href="{{ route('admin.course-lesson.add') }}" class="btn btn-outline-primary btn-sm">
...
<script type="text/javascript">
   $(document).ready(function() {
       var table = $('#myTable').DataTable({
           ...
           searching: true,
           ajax: {
               url: "{{ route('admin.course-lesson.getRecords') }}",
               data: function (d) {
           ...
           columns: [
               ...
               { data: 'course', name: 'course', title: 'Course Name' },  
               { data: 'lesson_name', name: 'lesson_name', title: 'Lesson Name' }, 
               { data: 'mode', name: 'mode', title: 'Lesson Mode' }, 
...
<script>
   ...
   $('#myTable').on('change', '.toggleStatus', function() {
      ...
      $.ajax({
          url: "{{ route('admin.course-lesson.changeStatus') }}",
          type: "POST",
          data: {
              _token: "{{ csrf_token() }}",
              id: id,
   ...
   confirmDelete(function() {
       $.ajax({
           url: "{{ route('admin.course-lesson.delete') }}",
           type: "POST",
           data: {
               id: id,
               _token: "{{ csrf_token() }}"
