                        ...
                        <div class="row g-3 align-items-end">
                           <!-- Role -->
                              ...
                              <select class="form-select  select2" id="filter_customer">
                                 <option value="">All Customers</option>
                                    @if(!empty($customer_list) && $customer_list->count() > 0)
                                        @foreach($customer_list as $value)
                                            <option value="{{ $value->id }}" {{ (isset($user_id) && $user_id == $value->id) ? 'selected' : '' }}> 
                                                {{ !empty($value->name) ? $value->name : 'N/A' }} -
                                                {{ !empty($value->phone_no) ? $value->phone_no : 'N/A' }}
                                            </option>
                                        @endforeach
                                    @endif
                              </select>
                           ...
                           <div class="col-12 col-md-6 col-lg-3">
                              ...
                              <select class="form-select select2" id="filter_survey_id">
                                 ...
                                 @if(!empty($survey_list) && $survey_list->count() > 0)
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select select2 w-100" id="assign_vendor_id" style="width: 100%;">
                        ...
                        @if(!empty($vendor_list) && $vendor_list->count() > 0)
                            @foreach($vendor_list as $v)
                                <option value="{{ $v->id }}">{{ !empty($v->name) ? $v->name : 'N/A' }} | {{ !empty($v->email_id) ? $v->email_id : 'N/A' }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
...
<script type="text/javascript">
   $(document).ready(function() {
       ...
       var table = $('#myTable').DataTable({
           ...
           searching: true,
           ajax: {
               url: "{{ route('admin.customer-survey.getRecords') }}",
               data: function (d) {
       ...
       $('#assignVendorBtn').on('click', function() {
           ...
           Swal.fire({
               ...
               if (result.isConfirmed) {
                   $.ajax({
                       url: "{{ route('admin.customer-survey.assignVendor') }}",
                       type: "POST",
                       data: {
                           id: id,
                           vendor_id: vendor_id,
