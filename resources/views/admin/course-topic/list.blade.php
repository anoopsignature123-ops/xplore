            ...
            <div class="card shadow-lg border-0 rounded-3">
               ...
               <div class="card-header bg-light border-bottom-0 py-3 d-flex justify-content-between align-items-center rounded-top">
                  ...
                  <div class="d-flex gap-2">
                     ...
                     @can('course-topic-add')
                        <a href="{{ route('admin.course-topic.add', $lesson_id) }}" class="btn btn-outline-primary btn-sm">
...
<script type="text/javascript">
   $(document).ready(function() {
       var table = $('#myTable').DataTable({
           ...
           searching: true,
           ajax: {
               url: "{{ route('admin.course-topic.getRecords') }}",
               data: function (d) {
           ...
           columns: [
               ...
               { data: 'topic_name', name: 'topic_name', title: 'Topic Name' }, 
...
<script>
   ...
   $('#myTable').on('change', '.toggleStatus', function() {
      ...
      $.ajax({
          url: "{{ route('admin.course-topic.changeStatus') }}",
          type: "POST",
          data: {
              _token: "{{ csrf_token() }}",
              id: id,
   ...
   confirmDelete(function() {
       $.ajax({
           url: "{{ route('admin.course-topic.delete') }}",
           type: "POST",
           data: {
               id: id,
               _token: "{{ csrf_token() }}"
