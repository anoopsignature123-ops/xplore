            ...
            <ul class="header-right"> 
             <li class="custom-dropdown">
              ...
              <li class="profile-nav custom-dropdown">
                <div class="user-wrap">
                  <div class="user-img"><img src="{{ !empty(auth('vendor')->user()->profile_image) ? asset(auth('vendor')->user()->profile_image) : asset($settings->favicon ?? '') }}" alt="{{ auth('vendor')->user()->name }}"></div>
                  <div class="user-content">
                    @if(auth('vendor')->check())
                    <h6>{{ auth('vendor')->user()->name }}</h6>
                    @endif
                    <p class="mb-0">Vendor<i class="fa-solid fa-chevron-down"></i></p>
                  </div>
                </div>
                <div class="custom-menu overflow-hidden">
                  <ul class="profile-body">
                    <li class="d-flex"> 
                      ...
                      </svg><a class="ms-2" href="{{ route('vendor.profile') }}">Profile</a>
                    </li>

                    <li class="d-flex"> 
                      <svg class="svg-color">
                        <use href="{{ config('app.admin_assets') }}svg/iconly-sprite.svg#Login"></use>
                      </svg><a class="ms-2" href="{{ route('vendor.logout') }}">Log Out</a>
                    </li>
