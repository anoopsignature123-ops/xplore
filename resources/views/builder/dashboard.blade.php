      ...
      <div class="page-title">
         <div class="row">
            ...
            <div class="col-sm-6 col-12 text-end">
               <a href="{{ route('builder.profile') }}" class="btn btn-outline-primary btn-sm"><i class="fas fa-user-edit me-1"></i> Edit Profile & Showcase</a>
               <a href="{{ route('builder.logout') }}" class="btn btn-outline-danger btn-sm ms-2"><i class="fas fa-sign-out-alt me-1"></i> Logout</a>
            </div>
         </div>
      </div>
   </div>
            ...
            <div class="card shadow-sm border-0 rounded-3 bg-primary text-white p-3">
               <div class="d-flex justify-content-between align-items-center">
                  <div>
                     ...
                     <h2 class="mb-0 fw-bold text-white">{{ $totalInquiries }}</h2>
                  </div>
            ...
            <div class="card shadow-sm border-0 rounded-3 bg-warning text-dark p-3">
               <div class="d-flex justify-content-between align-items-center">
                  <div>
                     ...
                     <h2 class="mb-0 fw-bold">{{ $pendingInquiries }}</h2>
                  </div>
            ...
            <div class="card shadow-sm border-0 rounded-3 bg-success text-white p-3">
               <div class="d-flex justify-content-between align-items-center">
                  <div>
                     ...
                     <h2 class="mb-0 fw-bold text-white">{{ $totalPortfolios }}</h2>
                  </div>
            ...
            <div class="card shadow-sm border-0 rounded-3 bg-info text-white p-3">
               <div class="d-flex justify-content-between align-items-center">
                  <div>
                     ...
                     <h2 class="mb-0 fw-bold text-white">{{ $totalProjects }}</h2>
                  </div>
               ...
               <div class="card-body">
                  <div class="table-responsive">
                     <table class="table table-striped table-bordered align-middle">
                        ...
                        <tbody>
                           @forelse($recentInquiries as $idx => $inq)
                              <tr>
                                 <td>{{ $idx + 1 }}</td>
                                 <td><strong><i class="fas fa-user me-1 text-primary"></i> {{ $inq->customer_name }}</strong></td>
                                 <td><i class="fas fa-phone-alt me-1 text-success"></i> <a href="tel:{{ $inq->customer_phone }}" class="fw-bold">{{ $inq->customer_phone }}</a></td>
                                 <td>
                                    @if($inq->inquiry_type == 'call')
                                       <span class="badge bg-success"><i class="fas fa-phone me-1"></i> Call Request</span>
                                    @elseif($inq->inquiry_type == 'chat')
                                 ...
                                 <td>{{ $inq->created_at ? $inq->created_at->format('d M Y, h:i A') : 'N/A' }}</td>
                                 <td>
                                    <span class="badge bg-{{ $inq->status == 'closed' ? 'success' : ($inq->status == 'contacted' ? 'info' : 'warning') }}">
                                       {{ ucfirst($inq->status) }}
                                    </span>
                                 </td>
                              </tr>
